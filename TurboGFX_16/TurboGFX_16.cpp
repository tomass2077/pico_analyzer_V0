...
_PSTL_PRAGMA_FORCEINLINE uint8_t expand5to8(uint8_t v) { return (v << 3) | (v >> 2); }
_PSTL_PRAGMA_FORCEINLINE uint8_t expand6to8(uint8_t v) { return (v << 2) | (v >> 4); }
_PSTL_PRAGMA_FORCEINLINE uint8_t compress8to5(uint8_t v) { return v >> 3; }
_PSTL_PRAGMA_FORCEINLINE uint8_t compress8to6(uint8_t v) { return v >> 2; }
// Adjusted Graphics class methods for 16-bit color
Graphics::Graphics(uint16_t *buffer, uint16_t width, uint16_t height)
    : buffer(buffer), width(width), height(height)
{
    buffer_size = width * height;
}

void Graphics::clear(Color_565 color)
{
    if (color.value == 0)
    ...
    else
    {
        ...
        uint16_t *endPtr = DATA + buffer_size;
        while (DATA != endPtr)
        {
            *DATA++ = color.value;
        }
...
void Graphics::fade(Color_565 color, uint8_t alpha)
{
    uint16_t *DATA = buffer;
    uint16_t *endPtr = DATA + buffer_size;
    ...
    while (DATA != endPtr)
    {
        *DATA = blendColors(color, Color_565(*DATA), alpha).value;
        DATA++;
    }
}

Color_565 Graphics::blendColors(Color_565 fgColor, Color_565 bgColor, uint8_t alpha)
{
    if (alpha == 0)
        return bgColor;
    if (alpha == 255)
    ...
    uint8_t fgR = expand5to8(fgColor.r());
    uint8_t fgG = expand6to8(fgColor.g());
    uint8_t fgB = expand5to8(fgColor.b());

    uint8_t bgR = expand5to8(bgColor.r());
    uint8_t bgG = expand6to8(bgColor.g());
    uint8_t bgB = expand5to8(bgColor.b());

    // Perform blending
    uint8_t invA = 255 - alpha;
    uint8_t outR = (fgR * alpha + bgR * invA) >> 8;
    uint8_t outG = (fgG * alpha + bgG * invA) >> 8;
    uint8_t outB = (fgB * alpha + bgB * invA) >> 8;

    return Color_565(outR, outG, outB);
}

void Graphics::swapColor(Color_565 &a, Color_565 &b)
{
    Color_565 temp = a;
    a = b;
    b = temp;
...
void Graphics::swapUInt16(uint16_t &a, uint16_t &b)
{
    uint16_t temp = a;
    a = b;
    b = temp;
...
int32_t Graphics::absInt32(int32_t x)
{
    return (x < 0) ? -x : x;
}
...
void Graphics::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, Color_565 color)
{
...
void Graphics::drawLineAA(float x0, float y0, float x1, float y1, Color_565 color)
{
...
// Rectangle drawing
void Graphics::drawRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, Color_565 color, bool fill)
{
    if (fill)
    {
...
// Circle drawing
void Graphics::drawCircle(uint16_t x0, uint16_t y0, uint16_t radius, Color_565 color, bool fill)
{
    int16_t x = radius;
    int16_t y = 0;
    int16_t err = 0;
...
void Graphics::drawCircleAA(float x0, float y0, float radius, Color_565 color, bool fill)
{
...
void Graphics::drawChar(int x, int y, char c, Color_565 color)
{
...
void Graphics::drawString(int x, int y, const char *str, Color_565 color)
{
    while (*str)
    {
